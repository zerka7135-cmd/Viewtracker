...
export default function AddAccountModal({ account, onClose, onAdded, onToast }) {
  ...
  const [bioLinkUrl, setBioLinkUrl] = useState(account?.bioLink?.url || '');
  ...
  const submit = async (e) => {
    ...
    try {
      const res = isEdit
        ? await updateAccountApi(account.name, trimmed, [igUrl.trim(), ttUrl.trim(), ytUrl.trim()], bioLinkUrl.trim())
        : await addAccountApi(trimmed, [igUrl.trim(), ttUrl.trim(), ytUrl.trim()], bioLinkUrl.trim());
      ...
      <div className="modal">
        <form className="card modal-card" onSubmit={submit}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <input className="input" placeholder="Lien en bio à tracker (optionnel)" value={bioLinkUrl} onChange={(e) => setBioLinkUrl(e.target.value)} />
            {account?.bioLink?.trackedUrl && (
              <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.5 }}>
                Lien à coller dans la bio : <span className="mono" style={{ color: 'var(--text-faint)' }}>{account.bioLink.trackedUrl}</span> ({account.bioLink.clicks} clic{account.bioLink.clicks !== 1 ? 's' : ''})
              </div>
            )}
          </div>
