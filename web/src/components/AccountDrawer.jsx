...
const PLATFORMS = [
  ...
  { key: 'tt', name: 'TikTok', color: '#00c2c7' },
...
export default function AccountDrawer({ account, onClose, onAccountsChanged, onToast }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Supprimer "${account.name}" ? Son historique de vues sera perdu.`)) return;
    try {
      ...
      <div className="drawer">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <button type="button" className="icon-btn icon-btn-danger" title="Supprimer" onClick={handleDelete}>
              <IconTrash size={15} />
            </button>
